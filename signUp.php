...
<?php 
$username = $name = $password = $repassword ="";
$userErr = $nameErr = $passwordErr = $repasswordErr ="";
$formint = 0;
...
$sql = "CREATE TABLE IF NOT EXISTS credentials(
    ...
    REGISTRATION DATE DEFAULT '".date("Y-m-d")."')";
$conn->query($sql);

/*$sql = "describe credentials";
    $r = $conn -> query($sql);
...
if ($formint == 4) {
    ...
    while ($row = $result->fetch_assoc()){
        foreach ($row as $x){
            if ($username == $x){
                $userErr='*This username is taken';
                $formint = $formint - 1;
            }
        }
    }
    if ($password != $repassword){
        $repasswordErr='*re-enetered password does not match';
        $formint = $formint - 1;
    }
}

if ($formint == 4){
    $sql = "INSERT INTO credentials(EMAIL, USERNAME, PASSWORD, REGISTRATION) VALUES ('".$name."', '".$username."', '".$password."', CURDATE())";
    $conn->query($sql);
...
?>
    ...
    <div class="wrapper">
        ...
        <form method='post' action='<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>' id='signupform'>
            <input type="text" placeholder="Username" name='username'> <br> <span class='error'> <?php echo $userErr;?></span>
            <input type="text" placeholder="Name" name='name'> <br> <span class='error'> <?php echo $nameErr;?></span>
            <input type="password" placeholder="Password" name='password'> <br> <span class='error'> <?php echo $passwordErr;?></span>
            <input type="password" placeholder="Re-Enter Password" name='repassword'> <br> <span class="error"> <?php echo $repasswordErr;?></span>
        <button type='submit' name='submit' form='signupform'>Sign Up</button>
        </form>
        <div class="member">
            Alreadya a member? <a href="login.php">Login Here</a>
        </div>
    </div>
